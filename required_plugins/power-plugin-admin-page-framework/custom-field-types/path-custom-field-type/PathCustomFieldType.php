...
if ( ! class_exists( 'PowerPlugin_PathCustomFieldType' ) ) :
include( dirname( __FILE__ ) . '/PowerPlugin_PathCustomFieldType_Node.php' );
/**
 ...
 * @version     0.0.1
 */
class PowerPlugin_PathCustomFieldType extends PowerPlugin_AdminPageFramework_FieldType_image {
    ...
    protected function construct() {
        ...
        add_action( 'wp_ajax_apf_path_field_type-power-plugin' , '__return_empty_string' );
    }
        ...
        return array(
            ...
            array(
                ...
                'translation'    => array(
                    ...
                    'nonce'     => $_sNonce,
                    'label'     => array(
                        'selectPath' => __( 'Select Path', 'power-plugin' ),
                        'select'     => __( 'Select', 'power-plugin' ),
                    ),
                ),
                'translation_var' => 'PowerPlugin_AdminPageFrameworkPathFieldType',
            ),
        );
    }
    ...
    public function getField( $aField ) {
        ...
        return $aField[ 'before_label' ]
            . "<div class='power-plugin-input-label-container power-plugin-input-container {$aField[ 'type' ]}-field'>" 
                . "<label for='{$aField[ 'input_id' ]}'>"
                    . $aField[ 'before_input' ]
                    . $this->getAOrB(
                        $aField[ 'label' ] && ! $aField[ 'repeatable' ],
                        "<span " . $this->getLabelContainerAttributes( $aField, 'power-plugin-input-label-string' ) . ">" 
                            . $aField[ 'label' ] 
                        . "</span>",                        
                        ''                        
                    )
         ...
         * @since       3.8.4
         * @since       3.9.0       Moved from `PowerPlugin_PathCustomFieldType`.
         * @param       array       $aField
    ...
    protected function doOnFieldRegistration( $aFieldset ) {
        ...
        private function ___handleQuery( $aFieldset ) {
            ...
            if ( empty( $_POST[ 'power-plugin_path_field_type' ] ) ||  empty( $_POST[ 'nonce' ] ) ) {
                return;
            }
            ...
            $_sRelativePath = empty( $_POST[ 'id' ] ) || '#' === $_sNodeID
                ? '/'
                : trim( $_sNodeID, '\\/' );
            $_oTreeNode     = new PowerPlugin_PathCustomFieldType_Node( $_sRelativePath, $_sRootDirPath, $_aPathOptions );
