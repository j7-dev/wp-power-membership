...
(function ( $ ) {
  ...
  function initializeJSTree() {
    ...
    tb_show(
      PowerPlugin_AdminPageFrameworkPathFieldType.label.selectPath,  // modal window title
    ...
    var _oButtonPanel  = $( '<div class="container-path-modal-select-button">'
      ...
      + '<div class="media-toolbar-primary search-form"><button type="button" class="button media-button button-primary button-small">' + PowerPlugin_AdminPageFrameworkPathFieldType.label.select + '</button></div>'
      + '</div>' );
        ...
        core: {
          ...
          data: function ( node, cb ) {
            $.ajax( {
              type: 'post',
              dataType: 'json',
              url: PowerPlugin_AdminPageFrameworkPathFieldType.ajaxURL,
              data: {
                id: node.id,
                action: 'apf_path_field_type-power-plugin',
                'power-plugin_path_field_type': 1,
                nonce: PowerPlugin_AdminPageFrameworkPathFieldType.nonce,
