(function($){
    ...
    $( document ).ready( function(){
        ...
        if ( 'undefined' === typeof PowerPlugin_AdminPageFrameworkSelect2FieldType ) {
            return;
        }
        debugLog( PowerPlugin_AdminPageFrameworkSelect2FieldType );
    ...
    function initializeSelect2( oNode ) {
        ...
        if ( _aOptions[ 'search_callback' ] ) {
            ...
            _aOptions = $.extend(
                ...
                {
                    ajax: {
                        url: PowerPlugin_AdminPageFrameworkSelect2FieldType.ajaxURL,
                        dataType: 'json',
                        ...
                        data: function (params) {
                            ...
                            return {
                                ...
                                doing_select2_ajax: PowerPlugin_AdminPageFrameworkSelect2FieldType.nonce, // ensure it is called from here
        ...
        if ( _aOptions[ 'width' ] && 'auto' !== _aOptions[ 'width' ] ) {
            var _oFieldContainer = _oSelect2Target.closest( '.power-plugin-field-select2' );
            _oFieldContainer.css( 'width', _aOptions[ 'width' ] );
            _oFieldContainer.children( '.power-plugin-select-label' )
                .css( 'width', '100%' );
            _oFieldContainer.children( '.power-plugin-select-label' )
                .children( 'label' )
                .children( '.power-plugin-input-container' )
                .css( 'width', '100%' );
            _aOptions[ 'width' ] = '100%';
        }
        ...
        if ( _aOptions[ 'search_callback' ] ) {
             ...
             * Set initial values.
             */
            var _oInputForEncoded = _oSelect2Target.closest( '.power-plugin-field' )
            ...
            _oSelect2Target.on( 'change', function( event ){
                ...
                $( this ).closest( '.power-plugin-field' )
                    .children( 'input[data-encoded]' ).first()
    ...
    function toggleElementVisibility( subject ) {
        ...
        var _oAllSubjects    = _bGlobal
            ? $( _sSelectors )
            : $( subject ).closest( '.power-plugin-section' ).find( _sSelectors );
        _oAllSubjects.hide();

        // Get the revealing elements
        var _oElementToReveal       = _bGlobal
            ? $( _sTargetSelector )
            : $( subject ).closest( '.power-plugin-section' ).find( _sTargetSelector );
    ...
    function templateResultForIcon( state ) {
        var _oIconDefinition = $( state.element ).closest( '.power-plugin-field' ).find( '.custom-icon-definition[data-value=' + state.id + ']' );
    ...
    function debugLog( ...message ) {
        if ( ! parseInt( PowerPlugin_AdminPageFrameworkSelect2FieldType.debugMode ) ) {
            return;
        }
