...
class PowerPlugin_AdminPageFramework_FieldType_table extends PowerPlugin_AdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'power-plugin-field-type-table', 'src' => dirname(__FILE__) . '/js/table.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'jquery-ui-accordion', 'power-plugin-script-form-main' ), 'translation_var' => 'PowerPlugin_AdminPageFrameworkFieldTypeTable', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'label' => array(), ), ), );
    }
    ...
    private function ___getLabel($aField)
    {
        ...
        return "<div class='power-plugin-input-label-container'>" . "<label for='" . esc_attr($aField[ 'input_id' ]) . "'>" . "<span " . $this->getLabelContainerAttributes($aField, 'power-plugin-input-label-string') . ">" . $aField[ 'label' ] . "</span>" . "</label>" . "</div>";
    }
    private function ___getTable($aField)
    {
        ...
        $_sCaption = $aField[ 'caption' ] ? $aField[ 'caption' ] : __('Set the caption with the <code>caption</code> argument.', 'power-plugin');
