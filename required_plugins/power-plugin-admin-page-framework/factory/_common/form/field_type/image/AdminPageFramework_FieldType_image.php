...
class PowerPlugin_AdminPageFramework_FieldType_image extends PowerPlugin_AdminPageFramework_FieldType {
    ...
    {
        return array( array( 'handle_id' => 'power-plugin-field-type-image', 'src' => dirname(__FILE__) . '/js/image.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'power-plugin-script-form-main' ), 'translation_var' => 'PowerPlugin_AdminPageFrameworkImageFieldType', 'translation' => array( 'fieldTypeSlugs' => $this->aFieldTypeSlugs, 'referer' => 'admin_page_framework', 'hasMediaUploader' => function_exists('wp_enqueue_media'), 'label' => array( 'uploadImage' => $this->oMsg->get('upload_image'), 'useThisImage' => $this->oMsg->get('use_this_image'), 'insertFromURL' => $this->oMsg->get('insert_from_url'), ), ), ), );
    }
...
jQuery(document).ready(function(){setPowerPlugin_AdminPageFrameworkImageUploader('{$sInputID}','true'==='{$_sRepeatable}','true'==='{$_bExternalSource}')})
JAVASCRIPTS;
        return "<script type='text/javascript' class='power-plugin-image-uploader-button'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>". PHP_EOL;
    }
...
JAVASCRIPTS;
        return "<script type='text/javascript' class='power-plugin-{$sType}-remove-button'>" . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . "</script>". PHP_EOL;
    }
