...
(function($){
  ...
  var apfContact = PowerPlugin_AdminPageFrameworkContactFieldType;
  ...
  function sendEmail( event ) {
    ...
    // Check required
    var _requiredInputs = $( this ).closest( '.power-plugin-sections' )
    ...
    var _oSpinner = $('<img src="' + apfMain.spinnerURL + '" alt="' + apfMain.messages.loading + '" />' )
      .addClass( 'power-plugin-ajax-spinner' );
    $( this ).closest( '.power-plugin-field' ).find( '.result-placeholder' ).prepend( _oSpinner );
    var _resultIcon = $( this ).closest( '.power-plugin-field' ).find( '.result-placeholder .dashicons' );
    ...
    $.ajax( {
        ...
        success: function ( response ) {
          ...
          if ( response.result ) {
            _resultIcon.addClass( 'dashicons-yes-alt success power-plugin-form-tooltip' );
            _resultIcon.prepend( "<span class='power-plugin-form-tooltip-content'>" + response.message + "</span>" );
          } else {
            _resultIcon.addClass( 'dashicons-warning error power-plugin-form-tooltip' );
            _resultIcon.prepend( "<span class='power-plugin-form-tooltip-content'>" + response.message + "</span>" );
          }
        },
        error: function( response ) {
          ...
          _resultIcon.addClass( 'dashicons-warning error power-plugin-form-tooltip' );
          _resultIcon.prepend( "<span class='power-plugin-form-tooltip-content'>" + response.status + ' ' + response.statusText + "</span>" );
        },
        complete: function() {
          _oSpinner.remove();
          debugLog( 'Request done.' );
          _resultIcon[ 'power-plugin-form-tooltip' ]({
              autoClose: false,
              shown: true,
              oneOff: true,
              noArrow: true,
