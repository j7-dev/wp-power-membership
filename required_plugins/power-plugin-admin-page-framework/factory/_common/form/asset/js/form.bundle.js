...
(function ( $ ) {
     ...
     */
    $.fn.selectAllPowerPlugin_AdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
     ...
     */
    $.fn.deselectAllPowerPlugin_AdminPageFrameworkCheckboxes = function() {
        jQuery( this ).parent()
            .find( 'input[type=checkbox]' )
            .prop( 'checked', false )
            .trigger( 'change' );   // 3.8.8+
...
( function( $ ) {
    ...
    var _removePowerPlugin_AdminPageFrameworkLoadingOutputs = function() {

        jQuery( '.power-plugin-form-loading' ).remove();
        jQuery( '.power-plugin-form-js-on' )
            .hide()
            .css( 'visibility', 'visible' )
            .fadeIn( 200 )
            .removeClass( '.power-plugin-form-js-on' );
    ...
    window.onerror = function(){
        ...
        _removePowerPlugin_AdminPageFrameworkLoadingOutputs();
...
(function ( $ ) {
    ...
    $.fn.aPowerPlugin_AdminPageFrameworkInputOptions = {};

    $.fn.storePowerPlugin_AdminPageFrameworkInputOptions = function( sID, vOptions ) {
        ...
        $.fn.aPowerPlugin_AdminPageFrameworkInputOptions[ sID ] = vOptions;
    };
    $.fn.getPowerPlugin_AdminPageFrameworkInputOptions = function( sID ) {
        sID = sID.replace( /__\d+_/, '___' ); // remove the section index
        return ( 'undefined' === typeof $.fn.aPowerPlugin_AdminPageFrameworkInputOptions[ sID ] )
            ? null
            : $.fn.aPowerPlugin_AdminPageFrameworkInputOptions[ sID ];
    }
...
(function ( $ ) {
    ...
    // Callback containers.
    $.fn.aPowerPlugin_AdminPageFrameworkAddRepeatableFieldCallbacks        = [];
    $.fn.aPowerPlugin_AdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    $.fn.aPowerPlugin_AdminPageFrameworkRemoveRepeatableFieldCallbacks     = [];
    $.fn.aPowerPlugin_AdminPageFrameworkSortedFieldsCallbacks              = [];
    $.fn.aPowerPlugin_AdminPageFrameworkStoppedSortingFieldsCallbacks      = [];
    $.fn.aPowerPlugin_AdminPageFrameworkAddedWidgetCallbacks               = [];
    $.fn.aPowerPlugin_AdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+

    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */
    $( document ).on( 'power-plugin_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aPowerPlugin_AdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registerPowerPlugin_AdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     *
     * Unlike the `power-plugin_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).on( 'power-plugin_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aPowerPlugin_AdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).on( 'power-plugin_stopped_sorting_sections', function( oEvent ){

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aPowerPlugin_AdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).on( 'power-plugin_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aPowerPlugin_AdminPageFrameworkRemoveRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $.fn.callBackSortedFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aPowerPlugin_AdminPageFrameworkSortedFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
    ...
    $.fn.callBackStoppedSortingFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aPowerPlugin_AdminPageFrameworkStoppedSortingFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
     ...
     */
    $( document ).on( 'power-plugin_saved_widget', function( event, oWidget ){
        $.each( $.fn.aPowerPlugin_AdminPageFrameworkAddedWidgetCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( 'function' !== typeof _hfCallback ) {
                return true;    // continue
     ...
     */
    $.fn.registerPowerPlugin_AdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aPowerPlugin_AdminPageFrameworkAddRepeatableFieldCallbacks.push(
            [ oCallbacks.added_repeatable_field, aFieldTypeSlugs ]
        );

        $.fn.aPowerPlugin_AdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
            [ oCallbacks.repeated_field, aFieldTypeSlugs ]
        );
        $.fn.aPowerPlugin_AdminPageFrameworkRemoveRepeatableFieldCallbacks.push(
            [ oCallbacks.removed_repeatable_field, aFieldTypeSlugs ]
        );
        $.fn.aPowerPlugin_AdminPageFrameworkSortedFieldsCallbacks.push(
            [ oCallbacks.sorted_fields, aFieldTypeSlugs ]
        );
        $.fn.aPowerPlugin_AdminPageFrameworkStoppedSortingFieldsCallbacks.push(
            [ oCallbacks.stopped_sorting_fields, aFieldTypeSlugs ]
        );
        $.fn.aPowerPlugin_AdminPageFrameworkAddedWidgetCallbacks.push(
            [ oCallbacks.saved_widget, aFieldTypeSlugs ]
        );

        // 3.8.0
        $.fn.aPowerPlugin_AdminPageFrameworkStoppedSortingSectionsCallbacks.push(
            [ oCallbacks.stopped_sorting_sections, aFieldTypeSlugs ]
        );

    };
    /**
     * An alias of the `registerPowerPlugin_AdminPageFrameworkCalbacks()` method.
    ...
    $.fn.registerAPFCallback = function( oCallbacks, aFieldTypeSlugs ) {
        $.fn.registerPowerPlugin_AdminPageFrameworkCallbacks( oCallbacks, aFieldTypeSlugs );
    }
...
(function ( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.power-plugin-fields.repeatable' ).each( function() {
            var _buttonModel = $( this ).siblings( '.repeatable-field-buttons-model' );
            if ( ! _buttonModel.length ) {
                return true;
            }
            var _aSettings           = _buttonModel.data();
            var _buttonContainer     = _buttonModel.children( '.power-plugin-repeatable-field-buttons' ).first();
            ...
            var _childFields = $( this ).find( '> .power-plugin-field.without-child-fields' );
            ...
            /* Otherwise, insert the button element at the beginning of the field tag */
                 ...
                 */
                if ( ! _childFields.find( '.power-plugin-repeatable-field-buttons' ).length ) {
             ...
             */
            $( this ).find( '> .power-plugin-field.with-child-fields' ).prepend( _buttonsNestedFields );
                /**
                 * Add buttons to the fields tag.
                 */
                // $( this ).find( '.power-plugin-field.with-nested-fields' ).prepend( _buttonsNestedFields );

                /**
                 * Support for inline mixed fields.
                 * @todo not sure why this is commented out. Remove this if it's okay.
                 */
                // $( this ).find( '.power-plugin-field.with-mixed-fields' ).prepend( _buttonsNestedFields );




            $( this ).updatePowerPlugin_AdminPageFrameworkRepeatableFields( _aSettings ); // Update the fields
     ...
     * @var PowerPlugin_AdminPageFrameworkScriptFormMain
     */
    var translation = PowerPlugin_AdminPageFrameworkScriptFormMain;
     ...
     */
    $.fn.updatePowerPlugin_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        var _oRepeatableButtons = $( nodeThis ).find( '.power-plugin-repeatable-field-buttons' )
            .filter( function() {
                return $( this ).closest( '.power-plugin-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            });
        ...
        _oRepeatableAddButtons.on( 'click', function() {
            ...
            $( this ).addPowerPlugin_AdminPageFrameworkRepeatableField();
        ...
        _oRepeatableRemoveButton.on( 'click', function() {
            ...
            $( this ).removePowerPlugin_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = _oRepeatableAddButtons.first().closest( '.power-plugin-field' ).attr( 'id' );
        var _nCurrentFieldCount = $( '#' + _sFieldsContainerID ).find( '.power-plugin-field' )
            .filter( function() {
                return $( this ).closest( '.power-plugin-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
            })
            .length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {
                $( '#' + _sFieldID ).addPowerPlugin_AdminPageFrameworkRepeatableField( _sFieldID );
            }
        }
     ...
     */
    $.fn.addPowerPlugin_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {

        if ( 'undefined' === typeof sFieldContainerID ) {
            sFieldContainerID = $( this ).closest( '.power-plugin-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.power-plugin-fields' );
        ...
        if ( ! _aOptions ) {
            var _nodeButtonContainer = nodeFieldContainer.find( '.power-plugin-repeatable-field-buttons' )
                .filter( function() {
                    return $( this ).closest( '.power-plugin-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                })
                .first();
        ...
        var _oInnerFields       = nodeFieldsContainer.find( '.power-plugin-field' )
                                    .filter( function() {
                                        return $( this ).closest( '.power-plugin-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
        var _oRepeatableButtons = nodeFieldContainer.find( '.power-plugin-repeatable-field-buttons' )
                                    .filter( function() {
                                        return $( this ).closest( '.power-plugin-fields' ).attr( 'id' ) === _sFieldsContainerID;  // Avoid dealing with nested field's elements.
                                    });
         ...
         */
        nodeNewField.updatePowerPlugin_AdminPageFrameworkRepeatableFields();
        ...
        nodeNewField.trigger(
            'power-plugin_added_repeatable_field',
            [
        ...
        $( nodeNewField ).find( '.power-plugin-field' ).addBack().trigger(
            'power-plugin_repeated_field',
            [
                0, // call type, 0 : repeatable fields, 1: repeatable sections
                jQuery( nodeNewField ).closest( '.power-plugin-fields' )    // model container
            ]
        );
        ...
        var nodeRemoveButtons = nodeFieldsContainer
            ...
            .filter( function() {
                return $( this ).closest( '.power-plugin-fields' ).attr( 'id' ) === _sFieldsContainerID;
            })
        ...
        nodeFieldsContainer.children( '.power-plugin-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.power-plugin-field,.power-plugin-fields,.power-plugin-fieldset' ).incrementAttributes(
                [ 'id', 'data-field_tag_id_model', 'data-field_id' ],
                _iFieldCount,
                _sFieldTagIDModel
            );
            oElement.find( '.power-plugin-fields' ).incrementAttributes(
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removePowerPlugin_AdminPageFrameworkRepeatableField = function() {

        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.power-plugin-field' );
        var nodeFieldsContainer = $( this ).closest( '.power-plugin-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        var _aOptions = nodeFieldsContainer.data( 'repeatable' );
...
( function( $ ) {
    ...
    $( document ).ready( function() {
        ...
        $( '.power-plugin-sections.repeatable-section' ).each( function( index ){

            var _buttonModel    = $( this ).children( '.repeatable-section-buttons-model' ).first();
            var _repeatOptions  = _buttonModel.data();
            _buttonModel = _buttonModel.find( '.power-plugin-repeatable-section-buttons' ).first().detach();

            // Add buttons
            $( this ).find( '.power-plugin-section-caption' ).each( function(){

                $( this ).show();
                var _oButtons = $( _buttonModel[0].outerHTML );
                if ( $( this ).children( '.power-plugin-collapsible-section-title' ).children( 'fieldset' ).length > 0 ) {
                    _oButtons.addClass( 'section_title_field_sibling' );
                }
                var _oCollapsibleSectionTitle = $( this ).find( '.power-plugin-collapsible-section-title' );
            ...
            // Update the fields
            $( this ).updatePowerPlugin_AdminPageFrameworkRepeatableSections( _repeatOptions );
     ...
     */
    $.fn.updatePowerPlugin_AdminPageFrameworkRepeatableSections = function( aSettings ) {

        var _oThis                = this;
        var _sSectionsContainerID = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.power-plugin-sections' ).attr( 'id' );

        // Store the sections specific options in an array.
        if ( ! $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions ) {
            $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions = [];
        }
        if ( ! $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions.hasOwnProperty( _sSectionsContainerID ) ) {
            $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ] = $.extend(
                {
        ...
        var _aOptions = $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions[ _sSectionsContainerID ];
        ...
        $( _oThis ).find( '.repeatable-section-add-button' ).on( 'click', function() {
            ...
            $( this ).addPowerPlugin_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // The Remove button behavior
        $( _oThis ).find( '.repeatable-section-remove-button' ).on( 'click', function() {
            $( this ).removePowerPlugin_AdminPageFrameworkRepeatableSection();
            return false; // will not click after that
        });

        // If the number of sections is less than the set minimum value, add sections.
        var _sSectionID           = _oThis.find( '.repeatable-section-add-button' ).first().closest( '.power-plugin-section' ).attr( 'id' );
        var _nCurrentSectionCount = jQuery( '#' + _sSectionsContainerID ).find( '.power-plugin-section' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentSectionCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentSectionCount ) > 0 ) {
                $( '#' + _sSectionID ).addPowerPlugin_AdminPageFrameworkRepeatableSection( _sSectionID );
            }
        }
     ...
     */
    $.fn.addPowerPlugin_AdminPageFrameworkRepeatableSection = function( sSectionContainerID ) {

        // Local variables
        if ( 'undefined' === typeof sSectionContainerID ) {
            sSectionContainerID = $( this ).closest( '.power-plugin-section' ).attr( 'id' );
        }
        var nodeSectionContainer    = $( '#' + sSectionContainerID );
        var nodeNewSection          = nodeSectionContainer.clone(); // clone without bind events.
        var nodeSectionsContainer   = nodeSectionContainer.closest( '.power-plugin-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.power-plugin-section-tabs-contents' )
            .children( '.power-plugin-section-tabs' )
            .first();

        var _iSectionIndex          = nodeSectionsContainer.attr( 'data-largest_index' );

        var _iFadein                = $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadein' ];
        var _iFadeout               = $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'fadeout' ];
        var _bPreserveValues        = $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'preserve_values' ];

        // If the set maximum number of sections already exists, do not add.
        var _sMaxNumberOfSections   = $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'max' ];
        if ( _sMaxNumberOfSections != 0 && nodeSectionsContainer.find( '.power-plugin-section' ).length >= _sMaxNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.power-plugin-repeatable-section-buttons' ).last();
        ...
        var _sSectionTabSlug = nodeNewSection.find( '.power-plugin-section-caption' ).first().attr( 'data-section_tab' );
        if ( ! _sSectionTabSlug || _sSectionTabSlug === '_default' ) {
            nodeNewSection.find( '.power-plugin-section-title' ).not( '.power-plugin-collapsible-section-title' ).hide();
        }
        ...
        if( 'function' === typeof nodeNewSection.enablePowerPlugin_AdminPageFrameworkCollapsibleButton ){
            nodeNewSection.find( '.power-plugin-collapsible-sections-title, .power-plugin-collapsible-section-title' ).enablePowerPlugin_AdminPageFrameworkCollapsibleButton();
        }
        ...
        $( nodeNewSection ).find( '.power-plugin-field' ).each( function( iFieldIndex ) {
            ...
            $( this ).updatePowerPlugin_AdminPageFrameworkRepeatableFields();
            ...
            $( this ).trigger(
                ...
                [
                    ...
                    jQuery( nodeNewSection ).closest( '.power-plugin-sections' )    // model container
                ]
            );
        ...
        nodeNewSection.updatePowerPlugin_AdminPageFrameworkRepeatableSections();

        // Rebind sortable fields - iterate sortable fields containers.
        nodeNewSection.find( '.power-plugin-fields.sortable' ).each( function() {
            $( this ).enablePowerPlugin_AdminPageFrameworkSortableFields();
        });
        ...
        if ( nodeTabsContainer.length > 0 && ! nodeSectionContainer.hasClass( 'is_subsection_collapsible' ) ) {
            ...
            nodeTabsContainer.closest( '.power-plugin-section-tabs-contents' ).createTabs( 'refresh' );
        ...
        var _incrementAttributes = function( oElement, iSectionsCount, oSectionsContainer ) {
            ...
            $( oElement ).find( 'tr.power-plugin-fieldrow, .power-plugin-fieldset, .power-plugin-fields, .power-plugin-field, table.form-table, input,textarea,select,option' )
                .incrementAttribute(
                    'id',
                    iSectionsCount,
                    _sSectionIDModel
            ...
            $( oElement ).find( '.power-plugin-fields' ).incrementAttribute(
        ...
        // @todo this may be able to be removed
            $( oElement ).find( '.power-plugin-fieldset' ).incrementAttribute(
                'data-field_id',
                iSectionsCount,
                _sSectionIDModel
            );
     ...
     */
    $.fn.removePowerPlugin_AdminPageFrameworkRepeatableSection = function() {

        // Local variables - preparing to remove the sections container element.
        var nodeSectionContainer    = $( this ).closest( '.power-plugin-section' );
        var sSectionContainerID     = nodeSectionContainer.attr( 'id' );
        var nodeSectionsContainer   = $( this ).closest( '.power-plugin-sections' );
        var sSectionsContainerID    = nodeSectionsContainer.attr( 'id' );
        var nodeTabsContainer       = $( this ).closest( '.power-plugin-section-tabs-contents' )
            .children( '.power-plugin-section-tabs' )
            .first();
        var nodeTabs                = nodeTabsContainer.children( '.power-plugin-section-tab' );
        ...
        var _sMinNumberOfSections = $.fn.aPowerPlugin_AdminPageFrameworkRepeatableSectionsOptions[ sSectionsContainerID ][ 'min' ];
        if ( _sMinNumberOfSections != 0 && nodeSectionsContainer.find( '.power-plugin-section' ).length <= _sMinNumberOfSections ) {
            var _nodeLastRepeaterButtons = nodeSectionContainer.find( '.power-plugin-repeatable-section-buttons' ).last();
        ...
        nodeSectionContainer.fadeOut( _iFadeout, function() {
            ...
            if ( 1 === _nodeRemoveButtons.length ) {
                ...
                var _sSectionTabSlug = nodeSectionsContainer.find( '.power-plugin-section-caption' ).first().attr( 'data-section_tab' );
                if ( ! _sSectionTabSlug || '_default' === _sSectionTabSlug ) {
                    nodeSectionsContainer.find( '.power-plugin-section-title' ).first().show();
                }
            }
        ...
        _oNextAllSections.each( function( _iIterationIndex ) {
            ...
            // Call the registered callback functions.
            // @deprecated  3.6.0
            // $( this ).find( '.power-plugin-field' ).each( function( iFieldIndex ) {
...
( function( $ ) {
  ...
  $( document ).ready( function() {
    ...
    $( '.power-plugin-section-tabs-contents' ).createTabs();
  });
...
(function($) {
    ...
    // Initialize
    $( document ).ready( function() {
        $( '.power-plugin-fields.sortable' ).each( function() {
            $( this ).enablePowerPlugin_AdminPageFrameworkSortableFields();
        } );
    });

    $.fn.enablePowerPlugin_AdminPageFrameworkSortableFields = function( sFieldsContainerID ) {
...
(function($) {
    ...
    // Initialization
    $( document ).ready( function() {
        $( '.power-plugin-sections.sortable-section' ).each( function(){
            $( this ).enablePowerPlugin_AdminPageFrameworkSortableSections();
        } );
    });

    $.fn.enablePowerPlugin_AdminPageFrameworkSortableSections = function( sSectionsContainerID ) {
        ...
        var _bIsTabbed      = _oTarget.hasClass( 'power-plugin-section-tabs-contents' );
        var _bCollapsible   = 0 < _oTarget.children( '.power-plugin-section.is_subsection_collapsible' ).length;

        _oTarget        = _bIsTabbed
            ? _oTarget.find( 'ul.power-plugin-section-tabs' )
            : _oTarget;
        ...
        var _aSortableOptions = {
                items: _bIsTabbed
                    ? '> li:not( .disabled )'
                    : '.power-plugin-section:not( .disabled, .power-plugin-collapsible-toggle-all-button-container )',
                handle: _bCollapsible
                    ? '.power-plugin-section-caption'
                    : false,
                ...
                stop: function(e,ui) {
                    ...
                    // Callback the registered callback functions.
                    $( this ).trigger(
                        'power-plugin_stopped_sorting_sections',
                ...
                // cancel: '.power-plugin-section-description, .power-plugin-section-title'
        ...
        if ( ! _bIsTabbed ) {
            ...
            _oSortable.on( 'sortstop', function() {
                ...
                $( this ).find( 'caption > .power-plugin-section-title:not(.power-plugin-collapsible-sections-title,.power-plugin-collapsible-section-title)' ).first().show();
                $( this ).find( 'caption > .power-plugin-section-title:not(.power-plugin-collapsible-sections-title,.power-plugin-collapsible-section-title)' ).not( ':first' ).hide();
  ...
  function debugLog( ...message ) {
    if ( ! parseInt( PowerPlugin_AdminPageFrameworkScriptFormMain.debugMode ) ) {
      return;
    }
...
(function($){
  ...
  $.widget( 'power-plugin.pointerTooltip', $.wp.pointer, {
   /**
 ...
 *
 * It provides the $[ 'power-plugin-tooltip' ]( options ) jQuery plugin method.
 * The name uses hyphens for the user's text domain to be replaced. So the compiled framework script will be entirely ported with without the keyword of `power-plugin`.
 *
 * To use the method,
 * ```
 * $( '.my-tooltip' )[ 'power-plugin-form-tooltip' ]( 'Hi, this is a tooltip content!' );
 * ```
 * Or have a text message enclosed in an element with a class `power-plugin-form-tooltip-content`.
 * ```
 *    <span class="my-tooltip dashicons dashicons-editor-help">
 *      <span class="power-plugin-form-tooltip-content">
 *        Some text
 *      </span>
 *    </span>
 * ```
 * Then, call it like
 * ```
 * $( '.my-tooltip' )[ 'power-plugin-form-tooltip' ]();
 * ```
 * Or
 * ```
 ...
 * ```
 *
 * If it the script is loaded, elements with the .power-plugin-from-tooltip class will be automatically parsed.
 * So not to call the `[ 'power-plugin-form-tooltip' ]()` method, just create an element with the selector
 * and it should automatically have a tooltip.
 *
 *
 * When the framework file is compiled, replace the keyword `power-plugin` with your text domain.
 */
(function($){

  // Initialize
  $( document ).ready( function() {
    $( '.power-plugin-form-tooltip' ).each( function() {
      $( this )[ 'power-plugin-form-tooltip' ]();
    } );
  } );

  $.fn[ 'power-plugin-form-tooltip' ] = function( options ) {
  ...
  function initialize( target, options ) {
    ...
    options = $.extend( {}, {
      pointerClass: 'power-plugin-form-tooltip-balloon',
  ...
  function handleTooltip( self, options ) {
    ...
    var _content   = 'undefined' !== typeof options.content
      ...
      : $( self ).find( '.power-plugin-form-tooltip-content' ).clone();
    ...
    var _options    = $.extend( true, {}, options, {
      ...
      position: {
        ...
        within: _offscreen ? _body : $( self ).closest( '.power-plugin-field, .power-plugin-fieldrow, .power-plugin-section' ),
      },
      buttons: function() {},
      close: function() {},
    }, options );
  ...
  function handleAutoClose( self, content ) {
      ...
      $( self ).add( '.power-plugin-form-tooltip-balloon' ).on( 'mouseleave', function( event ){
      ...
      $( self ).add( '.power-plugin-form-tooltip-balloon' ).on( 'mouseenter', function(){
    ...
    $( document ).ready( function() {
        ...
        $( document ).ajaxComplete( function( event, XMLHttpRequest, ajaxOptions ) {
            ...
            if( _aRequest.action && ( 'save-widget' === _aRequest.action ) ) {
                ...
                if ( $( _oWidget ).find( '.power-plugin-sectionset' ).length <= 0 ) {
                    return;
                }
