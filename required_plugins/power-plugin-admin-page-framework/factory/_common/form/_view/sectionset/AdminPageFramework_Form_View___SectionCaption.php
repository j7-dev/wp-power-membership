...
class PowerPlugin_AdminPageFramework_Form_View___SectionCaption extends PowerPlugin_AdminPageFramework_FrameworkUtility {
    ...
    private function _getCaption(array $aSectionset, $iSectionIndex, $aFieldsets, $aFieldErrors, $aFieldTypeDefinitions, $aCallbacks, $oMsg)
    {
        if (! $aSectionset[ 'description' ] && ! $aSectionset[ 'title' ]) {
            return "<caption class='power-plugin-section-caption' style='display:none;'></caption>";
        }
        $_oArgumentFormatter = new PowerPlugin_AdminPageFramework_Form_Model___Format_CollapsibleSection($aSectionset[ 'collapsible' ], $aSectionset[ 'title' ], $aSectionset);
        $_abCollapsible = $_oArgumentFormatter->get();
        $_oCollapsibleSectionTitle = new PowerPlugin_AdminPageFramework_Form_View___CollapsibleSectionTitle(array( 'title' => $this->getElement($_abCollapsible, 'title', $aSectionset[ 'title' ]), 'tag' => 'h3', 'section_index' => $iSectionIndex, 'collapsible' => $_abCollapsible, 'container_type' => 'section', 'sectionset' => $aSectionset, ), $aFieldsets, $this->aSavedData, $this->aFieldErrors, $aFieldTypeDefinitions, $oMsg, $aCallbacks);
        $_bShowTitle = empty($_abCollapsible) && ! $aSectionset[ 'section_tab_slug' ];
        return "<caption " . $this->getAttributes(array( 'class' => 'power-plugin-section-caption', 'data-section_tab' => $aSectionset[ 'section_tab_slug' ], )) . ">" . $_oCollapsibleSectionTitle->get() . $this->getAOrB($_bShowTitle, $this->_getCaptionTitle($aSectionset, $iSectionIndex, $aFieldsets, $aFieldTypeDefinitions), '') . $this->_getCaptionDescription($aSectionset, $aCallbacks[ 'section_head_output' ]) . $this->_getSectionError($aSectionset, $aFieldErrors) . "</caption>";
    }
    private function _getSectionError($aSectionset, $aFieldErrors)
    {
        ...
        return $_sSectionError ? "<div class='power-plugin-error'><span class='section-error'>* " . $_sSectionError . "</span></div>" : '';
    }
    private function _getCaptionTitle($aSectionset, $iSectionIndex, $aFieldsets, $aFieldTypeDefinitions)
    {
        $_oSectionTitle = new PowerPlugin_AdminPageFramework_Form_View___SectionTitle(array( 'title' => $aSectionset[ 'title' ], 'tag' => 'h3', 'section_index' => $iSectionIndex, 'sectionset' => $aSectionset, ), $aFieldsets, $this->aSavedData, $this->aFieldErrors, $aFieldTypeDefinitions, $this->oMsg, $this->aCallbacks);
        return "<div " . $this->getAttributes(array( 'class' => 'power-plugin-section-title', 'style' => $this->getAOrB($this->_shouldShowCaptionTitle($aSectionset, $iSectionIndex), '', 'display: none;'), )). ">" . $_oSectionTitle->get() . "</div>";
    }
    private function _getCaptionDescription($aSectionset, $hfSectionCallback)
    {
        ...
        $_oSectionDescription = new PowerPlugin_AdminPageFramework_Form_View___Description($aSectionset[ 'description' ], 'power-plugin-section-description');
        return "<div class='power-plugin-section-description'>" . call_user_func_array($hfSectionCallback, array( $_oSectionDescription->get(), $aSectionset )) . "</div>";
    }
