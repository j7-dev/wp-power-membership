...
class PowerPlugin_AdminPageFramework_AdminNotice extends PowerPlugin_AdminPageFramework_FrameworkUtility {
    ...
    public function __construct($sNotice, array $aAttributes=array( 'class' => 'error' ), array $aCallbacks=array())
    {
        ...
        $this->aAttributes[ 'class' ] = $this->getClassAttribute($this->aAttributes[ 'class' ], 'power-plugin-settings-notice-message', 'power-plugin-settings-notice-container', 'notice', 'is-dismissible');
        $this->aCallbacks = $aCallbacks + $this->aCallbacks;
        new PowerPlugin_AdminPageFramework_AdminNotice___Script;
        if (! $sNotice) {
            return;
        }
