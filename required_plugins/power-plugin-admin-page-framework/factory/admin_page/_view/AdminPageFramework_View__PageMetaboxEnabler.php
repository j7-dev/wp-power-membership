...
class PowerPlugin_AdminPageFramework_View__PageMetaboxEnabler extends PowerPlugin_AdminPageFramework_FrameworkUtility {
    public $oFactory;
    ...
    {
        $_aPageMetaBoxClasses = $this->getElementAsArray($GLOBALS, array( 'aPowerPlugin_AdminPageFramework', 'aMetaBoxForPagesClasses' ));
    ...
    {
        $_bLoaded = $this->getElement($GLOBALS, array( 'aPowerPlugin_AdminPageFramework', 'bAddedMetaBoxScript' ), false);
        if ($_bLoaded) {
            return;
        }
        $GLOBALS[ 'aPowerPlugin_AdminPageFramework' ][ 'bAddedMetaBoxScript' ] = true;
        $_sScript = <<<JAVASCRIPTS
jQuery(document).ready(function(){postboxes.add_postbox_toggles(pagenow)})
JAVASCRIPTS;
        echo '<script class="power-plugin-insert-metabox-script">' . '/* <![CDATA[ */' . $_sScript . '/* ]]> */' . '</script>';
    }
}
