...
final class View {
	...
	public function __construct() {
		...
		\add_action( 'save_post_' . self::POST_TYPE, [ $this, 'delete_transient', 100, 3 ] );
	}
	...
	public function init(): void {
		...
		if ( $power_plugins_settings[ Setting::ENABLE_SHOW_AVAILABLE_COUPONS_FIELD_NAME ] ) {
			...
			\add_filter(
				...
				[ $this, 'modify_minimum_amount_condition' ],
				200,
				3
			);
	...
	public function enqueue_assets(): void {
		if ( \is_checkout() ) {
			...
			\wp_enqueue_script(
				'handle-coupon',
				Plugin::$url . '/assets/js/handle-coupon.js',
				[ 'wc-checkout' ],
				Plugin::$version,
				true
			);
	...
	public static function get_order_quantity_by_user( int $user_id = 0 ): int {
		...
		global $wpdb;
		$query = $wpdb->prepare(
			"SELECT COUNT(ID) FROM {$wpdb->prefix}posts WHERE post_type = 'shop_order' AND post_status IN ('wc-completed', 'wc-processing') AND post_author = %d",
			$user_id
		);
