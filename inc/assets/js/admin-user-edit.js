(function () {
  ...
  let selector_inputHidden = document.querySelectorAll(
    'form#your-profile input[type="hidden"]'
  );
  ...
  selector_copy_OuterTR = selector_copy_OuterTR.map((item) =>
    document.querySelector(item)
  );
  selector_copy_OuterTR = selector_copy_OuterTR.filter((element) => {
    return element !== null;
  });
  selector_copy_OuterTR = selector_copy_OuterTR.map(
    (item) => item.closest("tr").outerHTML
  );

  const node_list_html =
    selector_copy_OuterTR.join("") + selector_copy.join("");
...
`;
  ...
  document.querySelector(".user-profile-picture img.avatar").style.cssText +=
